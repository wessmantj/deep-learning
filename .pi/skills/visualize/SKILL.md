---
name: visualize
description: "Add a correct, minimal visual to a lesson — a diagram or geometric picture — that renders inline in VS Code's markdown preview. Use when an idea is genuinely clearer as a picture: a dependency graph, system/flow, sequence, state machine, tree, comparison, or a spatial/geometric thing (coordinate geometry, number line, vectors, a plot, a physical layout). Outsources authoring+rendering to a maker subagent that verifies the image by looking at it, then you embed the returned file."
---

# Visualize

A picture earns its place only when it shows something words can't — shape, structure, direction, relationship, geometry. This skill produces ONE such picture, guarantees it is **correct** (the maker renders it and looks at it before returning), and drops it into the lesson so it renders inline in VS Code's markdown preview of the `md-log` file.
...
The maker owns its own purpose-built tools (`write_*`/`edit_*`/`render_*`) — it authors the source, renders it to a PNG, **looks at the PNG and iterates until it is correct and clean**, publishes it into the project's `viz/` folder with a unique filename, and returns:
...
Put the embed directly in your teaching reply, using a standard HTML `<img>` tag whose `src` is the returned **filename** under a path *relative to the lesson file*, plus a display width:

```
<img src="../viz/viz-<slug>-<timestamp>.png" width="500">
```

That's all. The `md-log` extension mirrors your reply text verbatim into the linked `.md`. Lesson files live in `lessons/` and the maker saves PNGs into `viz/`, both at the repo root — so from a lesson the picture is one directory up and over: `../viz/<filename>`. This relative path renders everywhere (VS Code's markdown preview, GitHub, any other renderer), unlike a root-relative `/viz/...` path which only resolves in VS Code. Use `<img>` (not markdown `![](...)`) so you keep the width control: `width="500"` is a good default; use larger for dense diagrams. Introduce the visual in a sentence, then let it carry the idea — don't narrate every element back in prose.
...
- The `../viz/<filename>` path points from the lesson file straight at the repo root's `viz/` folder where the maker published, so the embed resolves to exactly that file in any renderer — and unique filenames keep successive renders from colliding.
