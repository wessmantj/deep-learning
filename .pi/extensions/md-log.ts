 ...
 * to be viewed rendered (e.g. in VS Code's markdown preview), so assistant text with $...$ math,
...
export default function mdLog(pi: ExtensionAPI) {
	...
	// Plain-markdown callout for VS Code's markdown preview. Obsidian's
	// `> [!type] title` syntax shows up as literal text there, so instead we emit
	// a bold header inside a blockquote. The `title` already carries every state
	// distinction (correct ✓ / incorrect ✗ / "I don't know" / cancelled), so the
	// bold header alone keeps them visually distinct; the `type` is no longer
	// needed for rendering.
	function callout(_type: string, title: string, bodyLines: string[]): string {
		const lines = [`> **${title}**`];
		if (bodyLines.length > 0) lines.push(">");
	...
	// User prose can contain code fences and $...$ math, which break inside a
	// blockquote — so keep the body un-quoted and lean on the bold header to set
	// it apart from the assistant's turns.
	...
	// Assistant prose is the lesson body: markdown, math, code, mermaid. Keep it
	// un-quoted for the same reason as userBlock; the bold header separates turns.
